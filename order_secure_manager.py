...
# DATABASE INITIATION 
with sqlite3.connect('secure_orders.db') as db:
    cursor = db.cursor()
...
def inititalWindow():
    ...
    password_lbl = Label(window, text="Choose a Master Password")
    password_lbl.config(anchor=CENTER)
    password_lbl.pack()
    ...
    def savePassword():
        if txt.get() == txt1.get():
            ...
            password_lbl.config(text="Passwords dont match")
